...
> [!IMPORTANT]
> 最新版本为 [`v1.6.7 (Build 21)`](https://github.com/suguxiaojie/codex-notch-monitor/releases/tag/v1.6.7)。Release 分别提供 Apple Silicon `arm64` 与 Intel `x86_64` 安装包，请按 Mac 处理器选择对应 DMG。
...
菜单栏以额度环和百分比作为稳定基线；运行态在其后显示当前阶段、项目和本轮 Token，本轮数值使用 K／M／B 缩写。空间不足时可选择按优先级逐级精简，或在固定宽度内轮换项目与本轮 Token；重置倒计时只在悬停和 Glance 中显示。项目名按实际文字宽度省略，多项目的 Token 始终按会话独立展示。
...
- Glance 继续提供自己的周／月／三月／半年观察周期，不改变 Monitor Center 各独立卡片的筛选状态。
...
- 第一张卡片固定为 Token 活动，支持“30 日／90 日／半年”，不再与趋势共用切换状态。
- Token 趋势紧随活动，支持“周／月／三月”；项目用量卡显示“跟随趋势 · 当前周期”并与趋势相邻。
- 显示 Token 总量及输入、输出、缓存拆分。
- 显示会话数、项目数和按项目聚合的用量排行。
- 项目统计使用完整路径作为稳定主键，显示名称优先使用 Codex 本地侧栏别名。
- 趋势按自然日分桶显示非累计折线，并驱动同周期的项目排行。
- 活动卡使用 Token 密度热图查看不同日期的活跃程度，并保持独立周期。
...
- 第一张成本活动卡使用“30 日／90 日／半年”显示每日成本密度，并承载全页账号范围。
- 成本趋势卡使用“日／周／月”显示 API 等价美元成本。
...
- 支持“全部／重置信号／额度动态”筛选。
- 普通动态不显示重复的“动态”标签；只有限额、储备、已兑现、重置和预告使用语义类型标签。
- 实时雷达、已归档证据和置信度属于状态信息，使用元数据文字或状态点，不再借用类型标签颜色。
- 恢复记录保持中性操作按钮与独立计数，不与预测置信度或内容类型共用胶囊样式。
...
- 菜单栏可选择自动、详细、精简或仅图标密度；自动模式按真实安全空间优先展示最完整的信息，只有发生拥挤时才逐级收缩，并在空间恢复后小步扩展。
- 菜单栏空间不足时可轮换项目与本轮 Token，或按状态优先级逐级精简；额度环与百分比保持稳定，悬停查看额度倒计时及各项目、会话的完整说明。
...
“安装与权限”的维护页把应用更新固定为第一张卡片：启动后会检查 GitHub Latest Release，App 持续运行时每 30 分钟自动检查一次，进入设置会复用 30 分钟内缓存，也支持手动立即检查；发现更新后会自动选择适用于 Apple Silicon 或 Intel Mac 的 DMG，展示更新说明、文件大小和 SHA-256 摘要，并提供“稍后提醒”“查看说明”和“下载 DMG”。检查时图标旋转，发现新版本时下载图标轻微呼吸；开启“减少动态效果”后自动停用持续动画。当前版本只负责引导下载和手动安装，不会自动替换 `/Applications`。只有高于当前语义版本的 Release 才会提示，只推送 `main` 或替换同版本资产不会被当作新版本。
